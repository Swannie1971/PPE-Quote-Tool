...
const Money = ({ value, currency = 'R', decimals = 0, className = '' }) => {
  const n = Number(value || 0);
  const s = n.toLocaleString('en-ZA', { minimumFractionDigits: decimals, maximumFractionDigits: decimals });
  return <span className={`num ${className}`}>{currency} {s}</span>;
};

const Pct = ({ value, decimals = 1, signed = false }) => {
  const n = Number(value || 0);
  const sign = signed && n > 0 ? '+' : '';
  return <span className="num">{sign}{n.toFixed(decimals)}%</span>;
};
...
Object.assign(window, { Icon, Brand, Money, Pct, StatusBadge, CategoryIcon });
